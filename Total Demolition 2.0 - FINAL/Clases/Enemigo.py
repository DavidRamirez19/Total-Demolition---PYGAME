...
class Enemigo(pygame.sprite.Sprite):
    ...
    def __init__(self, ancho,alto,posx,posy,esTanque,tankFuerte,esJefe):
        ...
        if esTanque and (esJefe == 1):
			self.velocidad = 5 #velocidad para jefe 1
        if esTanque and (esJefe == 2):
			self.velocidad = 7 #velocidad para jefe 2			
        ...
        if self.esJefe == 1:
			self.rangoDisparo = 20
			self.rangoCambioMov = 5
        if self.esJefe == 2:
			self.rangoDisparo = 50
			self.rangoCambioMov = 5
    ...
    def __cargarVehiculo(self,grados):
        ...
        if self.esTanque and self.tankFuerte and self.esJefe ==2 and (self.resistencia == 6 or self.resistencia == 5):
			return pygame.transform.rotate(pygame.image.load('Imagenes/tankJefe2Golpe1.png'),grados)
        elif self.esTanque and self.tankFuerte and self.esJefe == 2 and (self.resistencia ==4 or self.resistencia == 3):
			return pygame.transform.rotate(pygame.image.load('Imagenes/tankJefe2Golpe2.png'),grados)
        elif self.esTanque and self.tankFuerte and self.esJefe == 2 and (self.resistencia == 2 or self.resistencia == 1):
    ...
    def __disparar(self):
        ...
        if (len(self.listaDisparo) < 2 and self.esTanque and self.esJefe == False): #solo puede haber 2 proyectiles lanzados activos por cada Tanque
            #self.sonidoDisparo.play()
            x,y = self.rect.center
            miProyectil = Proyectil.Proyectil(x,y,self.orientacion)
            self.listaDisparo.append(miProyectil)
        if  (len(self.listaDisparo) < 4 and self.esTanque and self.esJefe == 1):
			x,y = self.rect.center #Proyectiles para jefe 1
			miProyectil = Proyectil.Proyectil(x,y,self.orientacion)
			self.listaDisparo.append(miProyectil)
        if  (len(self.listaDisparo) < 6 and self.esTanque and self.esJefe == 2):
			x,y = self.rect.center # Proyectiles para jefe 2
			miProyectil = Proyectil.Proyectil(x,y,self.orientacion)
			self.listaDisparo.append(miProyectil)
